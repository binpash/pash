...
(cd "$PASH_TOP" && /bin/bash testing/nolocale_test_allonec2/merger.sh "" "$FOLDER_ID") &
MERGER_PID=$!

echo "  - Lambda1 (REMOTE): receiving from connection 50, sorting, sending to ed" 
(cd "$PASH_TOP" && /bin/bash testing/nolocale_test_allonec2/l1.sh "" "$FOLDER_ID") &
LAMBDA_PID=$!

echo "  - Lambda2 (REMOTE): receiving from connection ed, sorting, sending to 4f"
(cd "$PASH_TOP" && /bin/bash testing/nolocale_test_allonec2/l2.sh "" "$FOLDER_ID") &
LAMBDA2_PID=$!
