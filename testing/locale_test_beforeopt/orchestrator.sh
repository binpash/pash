...
echo "STEP 0: Delete RDV entry for connection ID '2b' (if exists)..."
aws dynamodb delete-item \
  --table-name rdv \
  --key '{"key": {"S": "2b"}}'
echo "  ✓ RDV entry deleted (if it existed)"
...
echo "STEP 0: Delete RDV entry for connection ID '4f' (if exists)..."
aws dynamodb delete-item \
  --table-name rdv \
  --key '{"key": {"S": "4f"}}'
echo "  ✓ RDV entry deleted (if it existed)"
...
echo "  - Splitter (LOCAL): splitting data and sending to connections 34 and 2b"
(cd "$PASH_TOP" && /bin/bash testing/locale_test_beforeopt/splitter.sh "" "$FOLDER_ID") &
SPLITTER_PID=$!

echo "  - Lambda1 (REMOTE): receiving from connection 34, sorting, sending to 17"
python3 "$INVOKE_SCRIPT" "$LAMBDA_SCRIPT_ID" "$FOLDER_ID" "lambda" &
LAMBDA_PID=$!

echo "  - Lambda2 (REMOTE): receiving from connection 2b, sorting, sending to 4f"
python3 "$INVOKE_SCRIPT" "$LAMBDA2_SCRIPT_ID" "$FOLDER_ID" "lambda" &
LAMBDA2_PID=$!

echo "  - Merger (LOCAL): receiving from connections 17 and 4f, merging output"
(cd "$PASH_TOP" && /bin/bash testing/locale_test_beforeopt/merger.sh "" "$FOLDER_ID") &
MERGER_PID=$!

echo "  - Waiting for all 4 nodes to complete..."
wait $SPLITTER_PID $LAMBDA_PID $LAMBDA2_PID $MERGER_PID
EXIT_CODE=$?
