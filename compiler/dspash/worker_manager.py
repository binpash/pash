...
from dspash.ir_helper import prepare_graph_for_remote_exec, to_shell_file, get_best_worker_for_subgraph, get_remote_writer_pipes, get_neighbor_remote_reader_pipes, update_subgraphs, get_worker_subgraph_map, update_remote_pipe_addr, add_debug_flags, DISCOVERY_PORT, report_exec_finish, get_remote_reader_pipes, update_remote_pipe_local_addr, is_remote_pipe
...
class WorkerConnection:
    ...
    def send_graph_exec_request(self, graph, shell_vars, functions, debug=False, naive_fault=False) -> bool:
        request_dict = { 'type': 'Exec-Graph',
                        'graph': graph,
                        'functions': functions,
                        'shell_variables': None, # Doesn't seem needed for now
                        'debug': None,
                        'naive_fault': naive_fault    
                    }
        if debug:
                ...
                try:
                    ...
                    while worker_subgraph_pairs:
                        ...
                        try:
                            ...
                            # Move merger subgraph id to the front so it will be updated the first (if it was scheduled on the crashed worker)
                            # TODO: find a more efficient way to find a merger subgraph i.e. is_merger_subgraph()
                            for i, subgraph_id in enumerate(subgraphs_id):
                                subgraph = self.subgraph_id_to_subgraph[subgraph_id]
                                if len(get_remote_reader_pipes(subgraph)) >= 1 and len(get_remote_writer_pipes(subgraph)) >= 1:
                                    # found the merger subgraph
                                    # swap to front
                                    log(f"merger subgraph id {subgraph_id} (previously on worker {self.subgraph_id_to_worker[subgraph_id]}) found at index {i}, moved to front")
                                    subgraphs_id[0], subgraphs_id[i] = subgraphs_id[i], subgraphs_id[0]
                                    break
                            ...
                            for subgraph_id in subgraphs_id:
                                ...
                                if worker == crashed_worker and not subgraph.is_finished():
                                    log(f"updating subgraph id {subgraph_id}")
                                    # Step 1
                                    ...
                                    # Step 2
                                    remote_writer_pipes = get_remote_writer_pipes(subgraph)
                                    ...
                                    # Step 3
                                    # TODO: if merger subgraph (contains immediate downstream neighbor) is also on the crashed_worker, don't try to update it
                                    ...
                                    # Step 4
                                    remote_reader_pipes = get_remote_reader_pipes(subgraph)
                                    for remote_reader_pipe in remote_reader_pipes:
                                        update_remote_pipe_local_addr(remote_reader_pipe, replacement_worker.host(), str(DISCOVERY_PORT))


                            # Remove all subgraphs from worker_subgraph_pairs that previously were assigned to the crashed worker
                            new_worker_subgraph_pairs = collections.deque([])
                            for i, (worker, subgraph) in enumerate(worker_subgraph_pairs):
                                if worker != crashed_worker:
                                    new_worker_subgraph_pairs.append([worker, subgraph])
                    ...
                    # TODO: may still need to find a way to update the main graph if the crash happens during io
