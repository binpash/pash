...
## TODO: For speculation, we don't want to invoke the compiler (for now) in (2),
##       we just want to block-ask the scheduler in (3) to let us know when the df_region
##       has finished executing and what is its exit code.

## TODO: In the future, we also want to let the scheduler know of any variable changes

## TODO: We might not need all the set state and other config done in (1) and (3) for speculative
