...
cat $stdin_redir | { time python3.8 $PASH_TOP/compiler/pash.py --speculation no_spec --output_optimized $eager_opt $auto_split_opt $config_path_opt --output_time $seq_script ; } 1> /tmp/distr_output/0 2> >(tee "${distr_result_filename}" >&2) &&
