...
cat $stdin_redir | { time python3 $PASH_TOP/compiler/pash.py --log_file /tmp/log_file -d 1 --speculation no_spec $assert_compiler_success $eager_opt $auto_split_opt $config_path_opt --output_time $seq_script ; } 1> /tmp/distr_output/0 2> >(tee "${distr_result_filename}" >&2) &&
