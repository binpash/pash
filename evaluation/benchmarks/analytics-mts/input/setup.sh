...
setup_dataset() {
  if [ ! -f ./in.csv ] && [ "$1" = "--full" ]; then
    ...
    curl -sf 'https://www.balab.aueb.gr/~dds/oasa-2021-01-08.bz2' --output - | bzip2 -d > in.csv
    if [ $? -ne 0 ]; then
    ...
    if [ ! -f ./in_small.csv ]; then                                                       
      ...
      curl -sf 'https://atlas-group.cs.brown.edu/data/covid-mts/in_small.csv.gz' | gunzip > in_small.csv
    fi                                                                                     
  fi
}
