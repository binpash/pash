...
do
    ...
    cat  |  tr -c 'A-Za-z' '[\n*]' | grep -v "^\s*$"| pure_func $input| sort | uniq -c > /dev/stdout |
    python3 "$SERVERLESS_RUNTIME_DIR/s3-put-object.py" "$OUT/$input.input.bigrams" /dev/stdin dummy # dummy object_id, not used for bash
done
