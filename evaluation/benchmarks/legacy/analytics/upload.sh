#!/bin/bash

cd "$(dirname "$0")" || exit 1

[ -z "$PASH_TOP" ] && {
  echo "PASH_TOP not set, maybe $(git rev-parse --show-toplevel)?"
  exit
}

[ -z "$AWS_BUCKET" ] && {
  echo "AWS_BUCKET not set"
  exit
}

BENCHMARK_DIR="$PASH_TOP/evaluation/benchmarks/analytics"
# INPUTS_DIR="$BENCHMARK_DIR/inputs"

S3_BUCKET_PREFIX="s3://$AWS_BUCKET"
S3_BENCHMARK_DIR="analytics"
S3_INPUTS_DIR="$S3_BENCHMARK_DIR/inputs"

INPUTS_DIRS=(
  nginx-logs_small
  nginx-logs_full
  pcaps_small
  pcaps_full
  port_scan_small
  port_scan_full
  ray_tracing_small
  ray_tracing_full
)

# upload routeviews.mrt file
ROUTEVIEWS="$BENCHMARK_DIR/inputs/routeviews.mrt"
if [ -f "$ROUTEVIEWS" ]; then
  S3_URI=$S3_BUCKET_PREFIX/$S3_INPUTS_DIR/routeviews.mrt
  echo "Uploading $ROUTEVIEWS to $S3_URI"
  aws s3 cp "$ROUTEVIEWS" "$S3_URI"
fi

# upload input directories
for INPUT_DIR in "${INPUTS_DIRS[@]}"; do
  for INPUT in $(ls $BENCHMARK_DIR/inputs/$INPUT_DIR); do
    INPUT_PATH="$BENCHMARK_DIR/inputs/$INPUT_DIR/$INPUT"
    S3_URI=$S3_BUCKET_PREFIX/$S3_INPUTS_DIR/$INPUT_DIR/$INPUT
    echo "Uploading $INPUT_PATH to $S3_URI"
    aws s3 cp "$INPUT_PATH" "$S3_URI"
  done
done
