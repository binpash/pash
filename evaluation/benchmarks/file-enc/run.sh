...
BENCHMARK_DIR="$PASH_TOP/evaluation/benchmarks/file-enc"
...
then
  ...
  SCRIPTS_INPUTS=(
    ...
    # nginx.sh:log_data
    # pcaps.sh:pcap_data
    # compress_files.sh:pcap_data
    encrypt_files.sh:pcap_data
  )
  INPUT_TYPE=""
fi
...
do
  ...
  do
    ...
    then
      ...
      { time IN=$S3_INPUT_PATH OUT=$S3_OUTPUT_PATH ENTRIES=10 "$PASH_TOP"/pa.sh -w "$WIDTH" -d 1 "$SCRIPT_PATH" --parallel_pipelines --serverless_exec; } 2>"$TIME_PATH"
    fi
