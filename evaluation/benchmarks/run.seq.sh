...
poets(){
  ...
  ./setup.sh
  cd ..
  echo 1syllable_words.sh:               $({ time ./6_4.sh > /dev/null; } 2>&1)
  echo 2syllable_words.sh:               $({ time ./6_5.sh > /dev/null; } 2>&1)
  echo 4letter_words.sh:                 $({ time ./6_2.sh > /dev/null; } 2>&1)
  echo bigrams_appear_twice.sh:          $({ time ./8.2_2.sh > /dev/null; } 2>&1)
  echo bigrams.sh:                       $({ time ./4_3.sh > /dev/null; } 2>&1)
  echo compare_exodus_genesis.sh:        $({ time ./8.3_3.sh > /dev/null; } 2>&1)
  echo count_consonant_seq.sh:           $({ time ./7_2.sh > /dev/null; } 2>&1)
# FIXME echo count_morphs.sh:                  $({ time ./7_1.sh > /dev/null; } 2>&1)
  echo count_trigrams.sh:                $({ time ./4_3b.sh > /dev/null; } 2>&1)
  echo count_vowel_seq.sh:               $({ time ./2_2.sh > /dev/null; } 2>&1)
  echo count_words.sh:                   $({ time ./1_1.sh > /dev/null; } 2>&1)
  echo find_anagrams.sh:                 $({ time ./8.3_2.sh > /dev/null; } 2>&1)
  echo merge_upper.sh:                   $({ time ./2_1.sh > /dev/null; } 2>&1)
  echo sort.sh:                          $({ time ./3_1.sh > /dev/null; } 2>&1)
  echo sort_words_by_folding.sh:         $({ time ./3_2.sh > /dev/null; } 2>&1)
  echo sort_words_by_num_of_syllables.sh:$({ time ./8_1.sh > /dev/null; } 2>&1)
  echo sort_words_by_rhyming.sh:         $({ time ./3.3 > /dev/null; } 2>&1)
#FIXME echo trigram_rec.sh:                   $({ time ./6_1.sh > /dev/null; } 2>&1)
  echo uppercase_by_token.sh:            $({ time ./6_1_1.sh > /dev/null; } 2>&1)
  echo uppercase_by_type.sh:             $({ time ./6_1_2.sh > /dev/null; } 2>&1)
  echo verses_2om_3om_2instances.sh:     $({ time ./6_7.sh > /dev/null; } 2>&1)
  echo vowel_sequencies_gr_1K.sh:        $({ time ./8.2_1.sh > /dev/null; } 2>&1)
  echo words_no_vowels.sh:               $({ time ./6_3.sh > /dev/null; } 2>&1)
  cd ..
}
