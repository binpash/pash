...
boxes <- 
  ggplot(bench[bench$shell %in% c('pash', 'blish') & !(bench$suite %in% c('for-loops', 'AvgTemp', 'WebIndex')),], 
...
bars <- 
  ...
  geom_hline(yintercept = 1) +
  labs(y = "", x = "", fill = "") +
  scale_fill_discrete(labels = c('Blish', 'PaSh')) +
  theme(axis.text.x = element_text(angle = 45, hjust=1),
        text = element_text(family='Times'),
...
boxes <-
  ggplot(bench[bench$shell %in% c('blish', 'blish -prof -par_pipe', 'blish -prof') & bench$suite %in% c('NLP'),],
  ...
  theme(axis.text.x = element_text(angle = 45, hjust=1),
        ...
        legend.position='none')
bars <-
  ggplot(bench[bench$shell %in% c('blish', 'blish -prof -par_pipe', 'blish -prof') & bench$suite %in% c('for-loops','AvgTemp'),],
         aes(x=factor(script, level=bar_order), y=as.double(speedup), fill=factor(shell))) +
  geom_col(position=position_dodge2(reverse=TRUE, width=1)) +
  geom_hline(yintercept = 1) +
  labs(y = "", x = "", fill = "") +
  scale_fill_discrete(labels = c('Blish', 'Blish no_prof', 'Blish no_prof no_du')) +
  theme(axis.text.x = element_text(angle = 45, hjust=1),
        text = text,
...
bench[!(bench$cshell %in% c('blish', 'blish_no_comm')),]$cshell <- 'ignore'
bench[bench$shell == 'blish_no_comm',]$cshell <- 'blish no_comm'
bench[bench$shell == 'blish',]$cshell <- 'blish'
...
bars <- 
  ...
  geom_col(position=position_dodge2(reverse=TRUE)) +
  geom_hline(yintercept = 1) +
  scale_fill_discrete(labels = c('Blish', 'Blish no_comm')) +
  labs(y = "", x = "", fill="") +
  theme(axis.text.x = element_text(angle = 45, hjust=1),
        text = text,
