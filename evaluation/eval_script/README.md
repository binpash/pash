...
* [Parallelizing Compilation Server] --  [compiler/pash_runtime_daemon.py](https://github.com/binpash/pash/blob/main/compiler/pash_runtime_daemon.py) handles compilation requests for parallelizing regions of the script. The compiler does hte following:
  ...
  - [Dependency untangling] -- If `--parallel_pipelines` flag is used, then the JIT Engine allows for running multiple regions at the same time; This increases the amount of parallelization that can be extracted from the script. The safety check for running a new regions [pash_runtime_daemon.py, L155](https://github.com/binpash/pash/blob/ebe379427a42040c1c00b01bcdcadb1fdbfb0281/compiler/pash_runtime_daemon.py#L155) is called after compilation succeeds [pash_runtime_daemon.py, L317](https://github.com/binpash/pash/blob/d836bfd32a58c848dccc2b157e72237f412c6ff5/compiler/pash_runtime_daemon.py#L317). Finally, the code responsible for executing the optimized script in parallel [pash_runtime.sh, L257](https://github.com/binpash/pash/blob/d836bfd32a58c848dccc2b157e72237f412c6ff5/compiler/pash_runtime.sh#L257)
  - [Profile Driven] -- If `--profile-driven` flag is used, then the compiler adjusts the parallelization factor based on previous execution times. The responsible code for determining the configuration [pash_runtime_daemon.py, L172](https://github.com/binpash/pash/blob/ebe379427a42040c1c00b01bcdcadb1fdbfb0281/compiler/pash_runtime_daemon.py#L172)
* [Commutativity Awareness] -- Consists of the following main components:
  - The annotations allow for adding the commutative property (e.g. [sort](https://github.com/binpash/pash/blob/ebe379427a42040c1c00b01bcdcadb1fdbfb0281/annotations/sort.json#L18))
