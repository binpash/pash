...
# 5.1: extract hello world --- kk: dish outputs single quotes as double quotes making this fail
...
# Better ways to implement the above (without awk)
...
# kk: ATM gives wrong result because tr -d '\n' is not stateless
cat $IN91 | tr ' ' '\n' | grep '[A-Z]' | tr '[a-z]' '\n' | grep '[A-Z]' | tr -d '\n' | cut -c 1-4
...
# Improved version
# cat $IN11 | cut -f 2 | cut -d ' ' -f 1 | sort | uniq -c | sort -nr | head -n 1  | tr -s ' ' ' ' | cut -d ' ' -f 3
