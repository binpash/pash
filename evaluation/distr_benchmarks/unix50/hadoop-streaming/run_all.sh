...
hadoop jar $jarpath -files 3.sh -D mapred.reduce.tasks=0 -D dfs.checksum.type=NULL -input $basepath/1.txt -output $outputs_dir/unix503 -mapper 3.sh && hadoop fs -cat $outputs_dir/unix503/part-* | head -n 2 | cut -d ' ' -f 2 > unix503_out.txt
...
hadoop jar $jarpath -files 20.sh -D mapred.reduce.tasks=0 -D dfs.checksum.type=NULL -input $basepath/8.txt -output $outputs_dir/unix5020 -mapper 20.sh && hadoop fs -cat $outputs_dir/unix5020/part-* | head -n 1 > unix5020_out.txt
