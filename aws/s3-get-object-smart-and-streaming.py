...
def stream_s3_to_fifo_handle(fifo_handle, bucket, key, start_byte, end_byte, debug=False):
    ...
    # 2. Pipe the stream directly to the FIFO handle.
    # copyfileobj reads from response['Body'] and writes to fifo_handle in chunks.
    # This blocks until done, but streams data continuously.
    # We use a large buffer (8MB) to maximize throughput.
