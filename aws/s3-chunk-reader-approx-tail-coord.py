...
"""
S3 chunk reader: approximate boundaries with tail coordination.

Design:
- EC2 sends approximate shard byte ranges.
- Each Lambda streams its shard immediately.
- For line correctness at shard boundaries, a Lambda can request "tail bytes"
  from later shards via pashlib + S3 rendezvous markers.
- `write_headers` controls whether output is framed with a 24-byte r_merge header.
"""
...
CHUNK_SIZE = int(os.environ.get("PASH_S3_CHUNK_SIZE", str(8 * 1024 * 1024)))
RECV_READ_SIZE = int(os.environ.get("PASH_S3_RECV_BUFFER", str(64 * 1024)))
...
def stream_s3_range_to_handle(handle, bucket, key, start_byte, end_byte, debug=False):
    """
    Stream S3 byte range directly to the provided file handle.
    ...
    if debug:
        print(f"[{_now_ts()}][STREAM] bytes={start_byte}-{end_byte}", file=sys.stderr)
        sys.stderr.flush()

    response = s3.get_object(Bucket=bucket, Key=key, Range=f"bytes={start_byte}-{end_byte}")
    shutil.copyfileobj(response["Body"], handle, length=CHUNK_SIZE)
...
def skip_first_partial_line(bucket, key, start_byte, end_byte, debug=False):
    ...
    if debug:
        print(f"[{_now_ts()}][SKIP] Looking for first newline at byte {start_byte}", file=sys.stderr)
        sys.stderr.flush()
    ...
    while start_byte + bytes_read <= end_byte:
        ...
        try:
            ...
            return start_byte + bytes_read + newline_pos + 1
        except ValueError:
...
def start_tail_server_thread(shard_index, job_uid, s3_bucket, bucket, key, start_byte, end_byte, debug=False):
    ...
    Runs in parallel with main shard streaming.
    """
...
def _serve_tail_bytes(
    ...
    try:
        ...
        if debug:
            print(f"[{_now_ts()}][SERVER {shard_index}] Ready for {requester_index}", file=sys.stderr)

        fifo_fd = os.open(send_fifo, os.O_WRONLY)
        with os.fdopen(fifo_fd, "wb", buffering=0) as fifo_handle:
            ...
            for chunk in response["Body"].iter_chunks(chunk_size=CHUNK_SIZE):
                try:
                    newline_pos = chunk.index(b"\n")
                    fifo_handle.write(chunk[: newline_pos + 1])
                    break
                except ValueError:
                    fifo_handle.write(chunk)

        proc.wait()
        os.unlink(send_fifo)

    except Exception as exc:
        print(f"[{_now_ts()}][SERVER {shard_index}] Error serving {requester_index}: {exc}", file=sys.stderr)
    finally:
...
def request_tail_bytes(shard_index, num_shards, job_uid, s3_bucket, debug=False):
    ...
    Called after main shard streaming.
    """
    ...
    for target_shard in range(shard_index + 1, num_shards):
        ...
        if not _wait_for_marker(s3, s3_bucket, ready_key, discovery_timeout, poll_secs, debug, f"[CLIENT {shard_index}]"):
            if debug:
                print(f"[{_now_ts()}][CLIENT {shard_index}] No reply from {target_shard}", file=sys.stderr)
            _s3_delete_marker(s3, s3_bucket, req_key)
            break
...
def main():
    try:
        ...
        with open(output_fifo, "wb", buffering=0) as fifo:
            ...
            if write_headers:
                # Header needs the final payload size up-front.
                tmp_payload = tempfile.TemporaryFile()
                log_timing("STREAM_START", "Streaming S3 to temp payload", debug)
                payload_bytes = stream_s3_range_to_handle(
                    tmp_payload,
                    s3_bucket,
                    s3_key,
                    actual_start_byte,
                ...
                log_timing("STREAM_END", f"Buffered {payload_bytes} payload bytes", debug)
                ...
                total_payload_bytes = payload_bytes + len(tail_bytes)
                write_block_header(fifo, shard_index, total_payload_bytes, 1)

                tmp_payload.seek(0)
                shutil.copyfileobj(tmp_payload, fifo, length=CHUNK_SIZE)
                if tail_bytes:
                    fifo.write(tail_bytes)
                fifo.flush()
                tmp_payload.close()
            else:
                log_timing("STREAM_START", "Streaming S3 directly to FIFO", debug)
                payload_bytes = stream_s3_range_to_handle(
                    fifo,
                    s3_bucket,
                    s3_key,
                ...
                log_timing("STREAM_END", f"Streamed {payload_bytes} payload bytes", debug)
        ...
        print(f"[{_now_ts()}][ERROR] Fatal exception: {exc}", file=sys.stderr)
        import traceback
